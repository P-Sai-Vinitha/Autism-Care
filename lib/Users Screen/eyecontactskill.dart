...
class EyeContactSkill extends StatefulWidget {
  final String email;
  const EyeContactSkill({Key? key, required this.email}) : super(key: key);

  @override
  State<EyeContactSkill> createState() => _EyeContactSkillState();
}

class _EyeContactSkillState extends State<EyeContactSkill> {
    ...
    showDialog(
      ...
      builder: (BuildContext context) {
        return AlertDialog(
          ...
          content: Text("Please move the sliders to answer the questions."),
          actions: <Widget>[
            TextButton(
              onPressed: () {
  ...
  Future<void> _fetchVideo() async {
    try {
      ...
      if (response.statusCode == 200) {
        ...
        final tempVideoFile = File('${tempDir.path}/eyecontact_video.mp4');
  ...
  Future<void> sendDataToBackend() async {
    String url = '$ip/eyecontact.php'; // Replace with your backend URL
...
void main() {
  runApp(MaterialApp(
    home: EyeContactSkill(email: 'example@example.com'), // Provide the email here
  ));
}
